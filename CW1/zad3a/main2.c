...
int main(){
    ...
    void * handle = dlopen("../zad1/libcontactlibshare.so", RTLD_LAZY);
    if (!handle) {
    ...
    createNewList = dlsym(handle,"createNewList");
    void (*addToList)(struct node * p, struct list * list1);
    addToList = dlsym(handle,"addToList");
    tree (*createNewTree)(comparation cmp);
    createNewTree = dlsym(handle,"createNewTree");
    tree (*addToTree)(contact cnt, tree tree);
    addToTree = dlsym(handle,"addToTree");
    void (*removeFromList)(struct list *list1, contact contact1);
    removeFromList = dlsym(handle,"removeFromList");
    tree (*deleteFromTree)(tree treearg, contact cnt);
    deleteFromTree = dlsym(handle,"deleteFromTree");
    struct node * (*findInList)(struct list *list1, contact contact1);
    findInList  =dlsym(handle,"findInList");
    struct list * (*sortList)(struct list *list1, comparation cmp);
    sortList = dlsym(handle,"sortList");
    tree (*changeComparationTree)(tree tree1, comparation cmp);
    changeComparationTree = dlsym(handle,"changeComparationTree");
    struct BSTNode * (*searchInTree)(tree tree1, contact cnt);
    searchInTree =dlsym(handle,"searchInTree");
